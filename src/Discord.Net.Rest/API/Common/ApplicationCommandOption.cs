...
namespace Discord.API
{
    internal class ApplicationCommandOption
    {
        ...
        public ApplicationCommandOption(IApplicationCommandOption cmd)
        {
            ...
            NameLocalizations = cmd.NameLocalizations?.ToDictionary(kvp => kvp.Key, kvp => kvp.Value) ?? Optional<Dictionary<string, string>>.Unspecified;
            DescriptionLocalizations = cmd.DescriptionLocalizations?.ToDictionary(kvp => kvp.Key, kvp => kvp.Value) ?? Optional<Dictionary<string, string>>.Unspecified;
        ...
        public ApplicationCommandOption(ApplicationCommandOptionProperties option)
        {
            Choices = option.Choices?.Select(x => new ApplicationCommandOptionChoice
            {
                Name = x.Name,
                Value = x.Value,
                NameLocalizations = x.NameLocalizations?.ToDictionary(kvp => kvp.Key, kvp => kvp.Value) ?? Optional<Dictionary<string, string>>.Unspecified,
            ...
            NameLocalizations = option.NameLocalizations?.ToDictionary(kvp => kvp.Key, kvp => kvp.Value) ?? Optional<Dictionary<string, string>>.Unspecified;
            DescriptionLocalizations = option.DescriptionLocalizations?.ToDictionary(kvp => kvp.Key, kvp => kvp.Value) ?? Optional<Dictionary<string, string>>.Unspecified;
        }
    }
}
